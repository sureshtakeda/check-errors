...
(function ($, Drupal) {
  ...
  $(document).on('click', '.image-wrapper a', function(e) {
    e.preventDefault();
    if($(this).hasClass('next')){
      var selector = 'nextAll';
      var next = '.hotspot-box:first';
    }else{
      var selector = 'prevAll';
      var next = '.hotspot-box:last';
    }
    var nav = $('.hotspot-box.active')[selector]('.hotspot-box:first');
    if(nav.length > 0 ){
      nav.click();
    }else{
      $(next).click();

...
Drupal.behaviors.takeda_taxonomy_term = {
  /**
   * Callback document loaded.
   */
  attach:function() {
    // Make sure that autopager plugin is loaded.
    var term_settings = drupalSettings.term_infinite_scroll.settings;
    if (typeof term_settings == 'object') {
        var $content_selector  = $('.related-content').not('.term-infinite-scroll-initialized');
        if ($content_selector[0]) {
          $content_selector.addClass('term-infinite-scroll-initialized');
          this.init_term_infinite_scroll(term_settings);
      }
    }
   ...
   */
  init_term_infinite_scroll: function(settings) {
    var content_selector  = '.related-content';
    var items_selector    = '.layout__region';
    var pager_selector    = 'ul.pager__items';
    var next_selector     = 'li.pager__item--next > a:first';
    var img_location      = '.related-content';
    var load_more         = 0;
    var load_more_markup  = '<button class="load-more">'+ settings.load_more_markup +'</button>';
    var ajax_pager        = 0;
    var img               = '';
    var $this = this;
    if (!$(content_selector)) {
    ...
    if (ajax_pager) {
      // Ajax pager
      autoloading = false;
      $(pager_selector + ' a').not('.term-infinite-scroll-ajax-initialized').click(function(e) {
        ...
        $.get(href, function (data) {
          ...
          $(content_selector).replaceWith(nextContent).addClass('term-infinite-scroll-initialized');
          Drupal.attachBehaviors($(content_selector)[0]);
        });
        e.preventDefault();
      }).addClass('term-infinite-scroll-ajax-initialized');
    }
    else {
      // Autopager
      $(pager_selector).hide();
      ...
      this.autopager(
        autopager_id,
        {
          ...
          load: function(current, next) {
            $(content_selector).next('div.term_infinite_scroll-ajax-loader').remove();
    ...
    if (!load_more && !ajax_pager && $(items_selector)[0]) {
      ...
      load_more_button.addClass('term-infinite-scroll-load-more')
